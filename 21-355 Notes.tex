...
\emph{Proof}: Let $P_1, P_2 \in \Pi[a,b]$. Then $L(P_1, f, \alpha) \leq L(P_1 \# P_2, f, \alpha) \leq U(P_1 \# P_2, f, \alpha) \leq U(P_2, f, \alpha)$ by last proposition. Hence $\underline{\int_a^b} f d\alpha = \sup\{L(P_1, f, \alpha) \;|\; P_1 \in \Pi[a,b]\} \leq U(P_2, f, \alpha)$. Then $\underline{\int_a^b} f d\alpha \leq \inf \{U(P_2, f, \alpha) \;|\; P_2 \in \Pi[a,b]\} = \overline{\int_a^b} f d\alpha$.
...
\textbf{Lemma}: Suppose $f \in \mathcal{R}([a,b]; \alpha)$. Define $F : [a,b] \to \mathbb{R}$ via $F(x) = \int_a^x f d\alpha$. Let $M = \sup \{|f(x)| \;|\; x \in [a,b]\}$. Then $|F(x) - F(y)| \leq M|\alpha(x) - \alpha(y)|$ ($\forall x,y \in [a,b]$).
...
Also, $\sup \{g(x) \;|\; x \in [x_{i-1}, x_i]\} = \sup \{(f \circ \phi)(x) \;|\; x \in [x_{i-1}, x_i]\} = \sup \{f(y) \;|\; y \in [\phi(x_{i-1}), \phi(x_i)\}$. Similarly, $\inf \{g(x) \;|\; x \in [x_{i-1}, x_i]\} = \inf \{f(y) \;|\; y \in [\phi(x_{i-1}), \phi(x_i)]\}$. Hence, since $\beta = \alpha \circ \phi$, $U(P, g, \beta) = U(\phi(P), f, \alpha)$ and $L(P, g, \beta) = L(\phi(P), f, \alpha)$ $\forall P \in \Pi[A,B]$.
...
\begin{quote}\vspace{-0.3cm}
	...
	\item If $\exists f : E \to \mathbb{R}$ such that $\forall n \geq l.\; \{|f_n(x) - f(x)| \;|\; x \in E\}$ is bounded, and $\lim_{n \to \infty} \sup \{|f_n(x) - f(x)|\} = 0$, then $f_n$ converges to $f$ \emph{uniformly} on $E$, and we write $f_n \rightrightarrows f$ as $n \to \infty$.
	\end{enumerate}
\end{quote}
